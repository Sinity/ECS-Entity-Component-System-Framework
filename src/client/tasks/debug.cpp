...
DebugTask::DebugTask(Engine& engine, Logger& logger, Configuration& config, Profiler& profiler) : 
    ...
    if(config.get("tasks.debugTask.log") == "true") {
        debugLogger.reset(new ConcreteLogger("DEBUG", "logz/debug.txt", false));
        debugLogger->loadSettings("tasks.debugTask.logger", config);
    } else {
        debugLogger.reset(new NullLogger);
    }
}
...
void DebugTask::receive(const ApplicationClosedEvent& UNUSED(appClosed)) {
