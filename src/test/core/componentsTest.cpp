...
BOOST_AUTO_TEST_CASE(component_container_intersection_test) {
    Logger& logger = *new NullLogger;
    Configuration& config = *new Configuration(logger);
	ComponentContainer components(logger, config);
...
BOOST_AUTO_TEST_CASE(component_container_creating_components_test) {
	...
	//these tests can be invalid if implementation changes
	BOOST_ASSERT(&c1 != &new_c1); //adress should change because of older entity created component.
